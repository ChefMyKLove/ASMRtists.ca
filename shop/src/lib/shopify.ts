...
function assertShopifyConfig() {
  if (SHOPIFY_STORE_PERMANENT_DOMAIN && SHOPIFY_STOREFRONT_TOKEN) return;
  throw new Error(
    "Missing Shopify Storefront configuration. Set VITE_SHOPIFY_STORE_DOMAIN and VITE_SHOPIFY_STOREFRONT_TOKEN.",
  );
}
    ...
    collections(first: $first) {
      edges {
        node {
          ...
          description
          image { url altText }
          products(first: 2) { edges { node { id images(first: 1) { edges { node { url altText } } } } } }
        }
      }
    }
  }
...
export const FEATURED_QUERY = `
  query GetFeatured {
    collectionByHandle(handle: "featured-collection") {
      products(first: 6) {
        edges {
          node {
            id
